...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['grades'])) {
    ...
    // Get school_year_id for the trigger validation
    ...
    if ($current_user_role !== 'admin') {
        ...
        $ta = $conn->prepare("SELECT grade_level, section FROM teacher_assignments WHERE teacher_id = ? AND assignment_type = 'adviser' AND school_year_id = ? LIMIT 1");
        $ta->bind_param("ii", $_SESSION['user']['id'], $school_year_id);
        $ta->execute();
    ...
    $all_final_ratings = [];
    $ga_result = $conn->query("SELECT id FROM subjects WHERE subject_name='General Average' LIMIT 1");
    $ga_subject_id = ($ga_result && $ga_result->num_rows > 0) ? $ga_result->fetch_assoc()['id'] : null;
    ...
    foreach ($_POST['grades'] as $subject_id => $data) {
        ...
        for ($q = 1; $q <= 4; $q++) {
            ...
            // Get old grade for history tracking
            $old_check = $conn->prepare("SELECT id, grade FROM grades 
                                        WHERE student_id = ? AND school_attended_id = ? 
                                        AND subject_id = ? AND quarter = ?");
            $old_check->bind_param("iiii", $_POST['student_id'], $_POST['school_attended_id'], $subject_id, $q);
            $old_check->execute();
            $old_result = $old_check->get_result()->fetch_assoc();
            $old_grade = $old_result ? $old_result['grade'] : null;
            $grade_id = $old_result ? $old_result['id'] : null;
            ...
            $stmt = $conn->prepare("INSERT INTO grades
                ...
                grade = VALUES(grade), final_rating = VALUES(final_rating), remarks = VALUES(remarks), teacher_id = VALUES(teacher_id)");
            $stmt->bind_param(
                "iiidissisi",
                $_POST['student_id'],
            ...
            // Get grade_id if this was an insert
            if (!$grade_id) {
                $grade_id = $conn->insert_id;
            }
            
            // Log change to history if grade changed
            if ($old_grade !== $grade) {
                $history_stmt = $conn->prepare("INSERT INTO grades_history 
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                $history_stmt->bind_param("iiiidddi", $grade_id, $_POST['student_id'], $_POST['school_attended_id'], 
                                         $subject_id, $q, $old_grade, $grade, $_SESSION['user']['id']);
    ...
    if (isset($_POST['remedial']) && is_array($_POST['remedial'])) {
        ...
        } elseif ($has_grade_level_col && $grade_level) {
            // Scope delete by grade_level so we don't wipe remedial from other grades
            $delete_stmt = $conn->prepare("DELETE FROM remedial_classes WHERE student_id = ? AND school_year = ? AND grade_level = ?");
            $delete_stmt->bind_param("iss", $_POST['student_id'], $school_year, $grade_level);
        } else {
    ...
    // Preserve the selected student and school record after redirect
    $_SESSION['restore_student'] = $_POST['student_id'];
    $_SESSION['restore_school'] = $_POST['school_attended_id'];
...
if (isset($_POST['ajax']) && $_POST['ajax'] === 'toggle_quarter_lock') {
    ...
    try {
        ...
        // Get school year from session (the one admin selected)
        $active_school_year = $_SESSION['school_year'] ?? null;
        
        // If no school year in session, fall back to active one
        if (!$active_school_year) {
            $sy_query = $conn->query("SELECT year FROM school_years WHERE is_active = 1 LIMIT 1");
            if ($sy_query && $sy_query->num_rows > 0) {
                $sy_row = $sy_query->fetch_assoc();
        ...
        // Check if record exists for this school year (global lock uses NULL for school_attended_id)
        $check = $conn->query("SELECT id FROM quarter_locks 
                              WHERE school_attended_id IS NULL 
                              AND quarter = $quarter 
                              AND school_year = '$active_school_year'");
        if (!$check) {
        ...
        if ($exists) {
            ...
            $result = $conn->query("UPDATE quarter_locks 
                                   SET locked = $locked 
                                   WHERE school_attended_id IS NULL 
                                   AND quarter = $quarter 
                                   AND school_year = '$active_school_year'");
        } else {
            // Insert new record (NULL for global lock, with school_year)
            $result = $conn->query("INSERT INTO quarter_locks (school_attended_id, quarter, locked, school_year) 
                                   VALUES (NULL, $quarter, $locked, '$active_school_year')");
        }
        ...
        logActivity($conn, $user['id'], 'UPDATE', 'quarter_locks', null, 
                   "Quarter $quarter $action for school year $active_school_year");
...
if (isset($_POST['ajax']) && $_POST['ajax'] === 'set_auto_lock_time') {
    ...
    if (!$active_school_year) {
        $sy_query = $conn->query("SELECT year FROM school_years WHERE is_active = 1 LIMIT 1");
        if ($sy_query && $sy_query->num_rows > 0) {
            $active_school_year = $sy_query->fetch_assoc()['year'];
        }
    }
    
    // Check if record exists (global lock uses NULL)
    $check = $conn->query("SELECT id FROM quarter_auto_locks WHERE school_attended_id IS NULL AND quarter = $quarter AND (school_year = '$active_school_year' OR school_year IS NULL)");
    $exists = $check->fetch_assoc();
    
    $result = false;
    if ($exists) {
        // Update existing record
        $result = $conn->query("UPDATE quarter_auto_locks SET auto_lock_time = '$lock_time' WHERE school_attended_id IS NULL AND quarter = $quarter AND (school_year = '$active_school_year' OR school_year IS NULL)");
    } else {
        // Insert new record (NULL for global)
        $result = $conn->query("INSERT INTO quarter_auto_locks (school_attended_id, quarter, auto_lock_time, school_year) VALUES (NULL, $quarter, '$lock_time', '$active_school_year')");
    }
...
if (isset($_POST['ajax']) && $_POST['ajax'] === 'set_auto_unlock_time') {
    ...
    // Check if record exists (global lock uses NULL)
    $check = $conn->query("SELECT id FROM quarter_auto_unlocks WHERE school_attended_id IS NULL AND quarter = $quarter AND (school_year = '$active_school_year' OR school_year IS NULL)");
    $exists = $check->fetch_assoc();
    
    $result = false;
    if ($exists) {
        // Update existing record
        $result = $conn->query("UPDATE quarter_auto_unlocks SET auto_unlock_time = '$unlock_time' WHERE school_attended_id IS NULL AND quarter = $quarter AND (school_year = '$active_school_year' OR school_year IS NULL)");
    } else {
        // Insert new record (NULL for global)
        $result = $conn->query("INSERT INTO quarter_auto_unlocks (school_attended_id, quarter, auto_unlock_time, school_year) VALUES (NULL, $quarter, '$unlock_time', '$active_school_year')");
    }
...
if (isset($_POST['ajax']) && $_POST['ajax'] === 'clear_auto_lock') {
    ...
    $result = $conn->query("DELETE FROM quarter_auto_locks WHERE school_attended_id IS NULL AND quarter = $quarter AND school_year = '$active_school_year'");
...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'get_subject_names') {
    ...
    // Get transfer status and grade info first (needed for grade_level)
    ...
    $grade_level_for_config = intval($school_info['grade_level'] ?? 0);

    // Fetch which MAPEH component subject IDs have a non-empty configured name for this grade
    $mapeh_component_ids = [9, 10, 11, 12];
    $configured_mapeh = [];
    if ($grade_level_for_config > 0) {
        $cfg_res = $conn->query("SELECT subject_id FROM subject_grade_groups
            WHERE subject_id IN (9,10,11,12)
            AND grade_level = $grade_level_for_config
            AND subject_name != ''");
        if ($cfg_res) {
            while ($cr = $cfg_res->fetch_assoc()) {
                $configured_mapeh[] = intval($cr['subject_id']);
            }
        }
    }
    ...
    while ($subject = $subjects_result->fetch_assoc()) {
        ...
        $is_mapeh_component = in_array(intval($subject['id']), $mapeh_component_ids);
        // A MAPEH component is "configured" only if it has a non-empty name in subject_grade_groups
        $is_configured = $is_mapeh_component ? in_array(intval($subject['id']), $configured_mapeh) : true;
        $subjects[] = [
            'id' => $subject['id'],
    ...
    echo json_encode([
        ...
        'grade_level' => $school_info['grade_level'],
        'adviser_name' => $school_info['adviser_name'] ?? null
    ]);
    exit;
}
...
if (isset($_POST['ajax']) && $_POST['ajax'] === 'clear_auto_unlock') {
    ...
    $result = $conn->query("DELETE FROM quarter_auto_unlocks WHERE school_attended_id IS NULL AND quarter = $quarter AND school_year = '$active_school_year'");
...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'get_student' && isset($_GET['id'])) {
    ...
    if ($user['role'] === 'teacher') {
        ...
        $query .= " ORDER BY grade_level ASC, section ASC, school_year DESC";
...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'load_grades') {
    ...
    // Get grades - use MAX(id) to get the most recent entry for each subject/quarter
    $grades_query = "SELECT g.subject_id, g.quarter, g.grade, g.final_rating, g.remarks 
                     FROM grades g
                     INNER JOIN (
    ...
    // Get remedial classes (scoped precisely to avoid cross-grade bleeding)
    $col_check = $conn->query("SHOW COLUMNS FROM remedial_classes LIKE 'school_attended_id'");
    $has_school_attended = ($col_check && $col_check->num_rows > 0);
    $col_check_gl = $conn->query("SHOW COLUMNS FROM remedial_classes LIKE 'grade_level'");
    $has_grade_level_col = ($col_check_gl && $col_check_gl->num_rows > 0);
    ...
    if ($has_school_attended) {
        ...
        $stmt = $conn->prepare("SELECT learning_area, final_rating, remedial_class_mark, 
                                       recomputed_final_grade, remarks, conducted_from, conducted_to 
        ...
        $stmt = $conn->prepare("SELECT learning_area, final_rating, remedial_class_mark, 
                                ...
                                WHERE student_id = ? AND school_year = (
                                    ...
                                    SELECT grade_level FROM schools_attended WHERE id = ?
                                )
                                ORDER BY id LIMIT 2");
        $stmt->bind_param("iii", $student_id, $school_attended_id, $school_attended_id);
    } else {
...
if ($user['role'] === 'teacher') {
    ...
    if ($adviser_info) {
        ...
        $students = $conn->query("SELECT id, UPPER(CONCAT(last_name, ', ', first_name)) AS fullname, lrn, gender, birthdate FROM students WHERE 1=0");
    }
} else {
    // Admin can see all students
    $students = $conn->query("SELECT id, UPPER(CONCAT(last_name, ', ', first_name)) AS fullname, lrn, gender, birthdate FROM students ORDER BY last_name");
}
...
/* Simplified single-student view */
#content-container {
...
.btn-save-grades.saving {
    background-color: #0d6efd;
    border-color: #0d6efd;
}

.btn-save-grades.saving .spinner-icon {
...
.btn-save-grades.saved {
    background-color: #198754;
    border-color: #198754;
}

.btn-save-grades.saved .bi-save::before {
    content: "\\f272"; /* bi-check-circle-fill */
}

.btn-save-grades.error {
...
<?php if (isset($_SESSION['restore_student']) && isset($_SESSION['restore_school'])): ?>
<script>
    // Restore student and school selection after save redirect
    document.addEventListener('DOMContentLoaded', function() {
        const studentId = <?= $_SESSION['restore_student'] ?>;
        const schoolId = <?= $_SESSION['restore_school'] ?>;
        
        console.log('Restoring selections - Student ID:', studentId, 'School ID:', schoolId);
        
        // Wait a bit for the page to fully load
        setTimeout(function() {
            // Find and click the student's row to load their grade form
            ...
            if (studentRow) {
                ...
                console.log('Loading student:', studentName);
                loadStudent(studentId, studentName);
                
                // Wait for grade form to load, then select the school
                setTimeout(function() {
                    const schoolSelect = document.getElementById('school-select-student-view');
                    if (schoolSelect) {
                        schoolSelect.value = schoolId;
                        
                        // Trigger change to load grades
                        const schoolChangeEvent = new Event('change', { bubbles: true });
                        schoolSelect.dispatchEvent(schoolChangeEvent);
                        
                        console.log('Selections restored successfully');
                    } else {
                        console.log('School select not found, retrying...');
                        // Try again after another delay
                        setTimeout(function() {
                            const schoolSelect2 = document.getElementById('school-select-student-view');
                            if (schoolSelect2) {
                                schoolSelect2.value = schoolId;
                                schoolSelect2.dispatchEvent(new Event('change', { bubbles: true }));
                                console.log('Selections restored on retry');
                            }
                        }, 1000);
                    }
                }, 1200);  // Wait longer for grade form to render
            } else {
                console.log('Student row not found');
            }
        }, 300);  // Initial delay for DOM to be ready
    });
</script>
<?php 
    unset($_SESSION['restore_student']); 
    unset($_SESSION['restore_school']); 
?>
<?php endif; ?>
...
<style>
  ...
  #student-view.grade-form-active {
    display: block;
  }
      ...
      <table class="table table-hover mb-0" id="studentsTable">
              ...
              <tbody>
                ...
                <tr class="student-row" 
                  ...
                  <td>
                    <button class="btn btn-sm btn-primary" onclick="loadStudentGrades(<?= $student['id'] ?>, '<?= htmlspecialchars($student['fullname'], ENT_QUOTES) ?>')">
      ...
      rows.sort((a, b) => {
        switch(this.value) {
          ...
          case 'gender-male':
            ...
            return a.getAttribute('data-name').localeCompare(b.getAttribute('data-name'));
          case 'gender-female':
            const genderA2 = a.getAttribute('data-gender');
            const genderB2 = b.getAttribute('data-gender');
            if (genderA2 === 'Female' && genderB2 !== 'Female') return -1;
            if (genderA2 !== 'Female' && genderB2 === 'Female') return 1;
            return a.getAttribute('data-name').localeCompare(b.getAttribute('data-name'));
          default:
            return 0;
...
function loadStudentGrades(studentId, studentName) {
  console.log('Loading grades for student:', studentId, studentName);
  loadStudent(studentId, studentName);
}
</script>
...
// Load selected student's grade form
function loadStudent(studentId, studentName) {
    // Hide student list card
    const studentCard = document.querySelector('.card');
    if (studentCard) {
        studentCard.style.display = 'none';
    }
    
    // Create or get grade form container
    let container = document.getElementById('grade-form-container');
    if (!container) {
        container = document.createElement('div');
        container.id = 'grade-form-container';
        container.className = 'card';
        container.style.cssText = 'overflow: hidden; border-radius: 0.375rem; margin-top: 1rem;';
        studentCard.parentNode.insertBefore(container, studentCard.nextSibling);
    }
    
    container.innerHTML = `
        <div class="card-header d-flex justify-content-between align-items-center">
            <span><i class="bi bi-pencil-square"></i> Grade Entry - ${studentName}</span>
            <button class="btn btn-sm btn-secondary" onclick="backToStudentList()">
                <i class="bi bi-arrow-left"></i> Back to Student List
            </button>
        </div>
        <div class="card-body" id="student-view" style="padding: 1.5rem;">
    ...
    fetch(`grades.php?ajax=get_student&id=${studentId}`)
        ...
        .then(data => {
            ...
            if (data.school_history && data.school_history.length > 0) {
                // Use 'student-view' as the fixed container ID for the grade form
                renderGradeForm('student-view', studentId, studentName, data.school_history);
            } else {
                studentView.innerHTML = `<div class="alert alert-warning">No school records found for this student.</div>`;
            }
        })
        .catch(error => {
            ...
            const studentView = document.getElementById('student-view');
            if (studentView) {
                studentView.innerHTML = '<div class="alert alert-danger">An error occurred while loading student data.</div>';
            }
        });
}

// Back to student list function
function backToStudentList() {
    const studentCard = document.querySelector('.card');
    const gradeContainer = document.getElementById('grade-form-container');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Clear all cached form data on page load to ensure fresh data from database
    // This prevents stale cached values from overriding database changes made by other users
    const storageKeys = Object.keys(sessionStorage);
    storageKeys.forEach(key => {
        if (key.includes('tabFormData_')) {
            sessionStorage.removeItem(key);
            console.log('Cleared cached form data:', key);
        }
    });
...
// Update URL parameters when school/grade changes (simplified for single student view)
function updateURLParameters(tabId) {
    // Only update URL for student-view
    if (tabId !== 'student-view') return;
    ...
    if (match) {
        ...
        const studentIdInput = document.querySelector(`#${tabId} input[name="student_id"]`);
...
function renderGradeForm(tabId, studentId, studentName, schoolHistory) {
    ...
    tabContent.innerHTML = `
        <form method="POST" id="form-${tabId}">
            ...
            <div class="selection-card">
                <div class="mb-3">
                    <h5 class="mb-0"><i class="bi bi-building"></i> ${studentName} - School Record</h5>
                </div>
                <div class="row g-3">
                    <div class="col-md-12">
            ...
            <div class="d-flex justify-content-end gap-2 mt-4">
                <button type="button" class="btn btn-secondary" onclick="refreshGrades('${tabId}', ${studentId})">
                    <i class="bi bi-arrow-clockwise"></i> Refresh
                </button>
                <button type="button" class="btn btn-primary btn-save-grades" id="save-btn-${tabId}" onclick="confirmSaveGrades('${tabId}')">
                    <i class="bi bi-check-circle"></i> <span class="save-btn-text">Save Grades</span>
                </button>
            </div>
            ...
            <div class="mt-3">
                ...
                <p class="text-muted small mb-0">
                    <i class="bi bi-info-circle"></i> <strong>Grade Status:</strong>
...
// Removed updateGradeIndicator - using autosave instead
...
function trackUnsavedChanges(tabId) {
    ...
    gradeInputs.forEach(input => {
        ...
        // REMOVED: Auto-save disabled to prevent conflicts
        // Users must click the Save button manually
    });
}

// Autosave functionality
let autosaveTimers = {}; // Store timers per tab

function triggerAutosave(tabId) {
    // Clear existing timer for this tab
    if (autosaveTimers[tabId]) {
        clearTimeout(autosaveTimers[tabId]);
    }
    
    // Realtime autosave - no delay
    performAutosave(tabId);
}

async function performAutosave(tabId) {
    console.log('🔄 Autosaving grades for', tabId);
    
    // Show saving status when actually saving
    ...
    try {
        ...
        if (response.ok) {
            console.log('✅ Autosave successful for', tabId);
            updateAutosaveStatus('saved');
            
            // Update original values after successful save
            const gradeInputs = document.querySelectorAll(`#${tabId} input[name^="grades["]`);
            gradeInputs.forEach(input => {
                if (!input.name.includes('subject_name') && !input.name.includes('final_rating') && !input.name.includes('remarks')) {
                    input.dataset.originalValue = input.value;
                }
            });
        } else {
...
function updateAutosaveStatus(status) {
    // Update all save buttons only - no floating toast notifications
    const saveButtons = document.querySelectorAll('.btn-save-grades');
    saveButtons.forEach(btn => {
        btn.classList.remove('saving', 'saved', 'error');
        
        let icon = btn.querySelector('.bi, .spinner-icon');
        ...
        switch(status) {
            ...
            case 'saved':
                ...
                setTimeout(() => {
                    ...
                    const currentIcon = btn.querySelector('.bi, .spinner-icon');
                    if (currentIcon) {
                        const saveIcon = document.createElement('i');
                        saveIcon.className = 'bi bi-save';
                        currentIcon.replaceWith(saveIcon);
                    }
                    if (textSpan) textSpan.textContent = 'Save Grades';
                }, 2000);
                break;
            case 'error':
                ...
                if (icon) {
                    ...
                    errorIcon.className = 'bi bi-exclamation-circle';
                    icon.replaceWith(errorIcon);
                }
...
// Removed markGradesAsSaved - using autosave instead
    ...
    try {
        ...
        if (data.success) {
            ...
            data.subjects.forEach(subject => {
                ...
                // Disable grade inputs for MAPEH components not configured in manage_subjects
                if (subject.is_configured === false) {
                    const row = document.querySelector(`#${tabId} input[name="grades[${subject.id}][subject_name]"]`)?.closest('tr');
                    if (row) {
                        row.querySelectorAll('input.grade-input').forEach(gi => {
                            gi.disabled = true;
                            gi.value = '';
                            gi.style.background = '#e9ecef';
                            gi.style.cursor = 'not-allowed';
                            gi.title = 'This subject is not configured for this grade level';
                        });
                        // Also grey out the subject name cell
                        const nameInput = row.querySelector('input[name*="subject_name"]');
                        if (nameInput) nameInput.style.color = '#adb5bd';
                        console.log(`  ✗ Subject ${subject.id} grade inputs DISABLED (not configured)`);
                    }
                }
            });
...
async function loadSavedGrades(tabId, studentId, skipSessionRestore = false) {
    ...
    try {
        ...
        if (data.success) {
                // First, clear all grade inputs if skipSessionRestore is true
                // This prevents browser form restoration from interfering
                if (skipSessionRestore) {
                    const allGradeInputs = document.querySelectorAll(`#${tabId} input[name^="grades["]`);
                    allGradeInputs.forEach(input => {
                        if (!input.name.includes('subject_name')) {
                            input.value = '';
                        }
                    });
                }
                ...
                // Store database values FIRST (before any sessionStorage restoration)
                // This is the source of truth for what's actually saved in the database
                const dbGrades = {};
                Object.keys(data.grades).forEach(subjectId => {
                    const gradeData = data.grades[subjectId];
                    for (let q = 1; q <= 4; q++) {
                        const fieldName = `grades[${subjectId}][q${q}]`;
                        if (gradeData['q' + q]) {
                            dbGrades[fieldName] = gradeData['q' + q];
                        }
                    }
                });
                
                console.log(`🗄️ Database grades (dbGrades) for ${tabId}:`, dbGrades);
                console.log(`📊 dbGrades has ${Object.keys(dbGrades).length} entries`);
                console.log(`⚙️ skipSessionRestore = ${skipSessionRestore}`);
                
                // Get current sessionStorage values
                const savedFormData = sessionStorage.getItem(storagePrefix + 'tabFormData_' + tabId);
                ...
                console.log(`🔍 Starting grade comparison for ${tabId}, skipSessionRestore=${skipSessionRestore}`);
                
                // Save these grades as the "original saved" values for comparison
                const savedGrades = {};
                const gradeInputs = document.querySelectorAll(`#${tabId} input[name^="grades["]`);
                
                console.log(`📊 Found ${gradeInputs.length} grade inputs in ${tabId}`);
                ...
                gradeInputs.forEach(input => {
                    ...
                    const dbValue = dbGrades[fieldName] || '';
                    
                    // Store database value as original (for autosave comparison)
                    if (currentValue) {
                ...
                console.log(`💾 Final savedGrades for ${tabId}:`, savedGrades);
                console.log(`📝 Storing ${Object.keys(savedGrades).length} saved grades in sessionStorage`);

...
if (!empty($search)) {
    $order_sql = "CASE 
        ...
        WHEN CONCAT_WS(' ', s.first_name, s.middle_name, s.last_name) = ? THEN 2
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateTable() {
        ...
        if (searchInput === document.activeElement) {
            sessionStorage.setItem('studentSearchFocus_sf10', 'true');
            sessionStorage.setItem('studentSearchCursor_sf10', searchInput.selectionStart);
        }
    ...
    window.addEventListener('load', function() {
        if (sessionStorage.getItem('studentSearchFocus_sf10') === 'true' && searchInput) {
            searchInput.focus();
            const cursorPos = sessionStorage.getItem('studentSearchCursor_sf10');
            if (cursorPos !== null) {
                searchInput.setSelectionRange(cursorPos, cursorPos);
            }
            sessionStorage.removeItem('studentSearchFocus_sf10');
            sessionStorage.removeItem('studentSearchCursor_sf10');
        }
    });
